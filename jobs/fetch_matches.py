...
last_matchday = get_last_stored_matchweek()
logger.info(f"LAST MATCHDAY: {last_matchday}")

for matchday in range(last_matchday, 39):
    ...
    with open(f'storage/matchday_{matchday}_data.json', 'w') as f:
        f.write(json.dumps(data, indent=4))
    matches_played = data["resultSet"]["played"]

    if not matches_played:
        logger.info(f"matchday {matchday} matches count is 0, breaking out of the loop")
        break

    logger.info(f"downloading matches from matchday {matchday} ({matches_played})")
